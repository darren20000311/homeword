...
class Pinky(GameCharacter):
    ...
    def update(self):
        ...
        # START CODE CHANGES
        if on_vert and on_horz: # Pinky is at an intersection
    
            if abs(up_down_part) > abs(left_right_part):
                # Pacman is more up/down of Pinky
                if up_down_part < 0: 
                    # Pacman is above Pinky
                    self.y_add = -self.velocity 
                    self.x_add = 0
                else:
                    # Pacman is below Pinky
                    self.y_add = self.velocity
                    self.x_add = 0

            else:
                # Pacman is more left/right of Pinky
                if left_right_part < 0:
                    # Pacman is left of Pinky
                    self.x_add = -self.velocity
                    self.y_add = 0
                else: 
                    # Pacman is right of Pinky
                    self.x_add = self.velocity
                    self.y_add = 0
