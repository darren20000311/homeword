...
class Maze:
    ...
    def eat_dots(self, pacman_x, pacman_y):
        all_dots = self.dots.top_row + self.dots.bottom_row + self.dots.left_col + self.dots.right_col
        dots_to_remove = all_dots[:]
        for dot in dots_to_remove:
            distance = dist(dot.x, dot.y, pacman_x, pacman_y)
            if distance < self.dots.EAT_DIST:
                self.dots.eat(dot)
